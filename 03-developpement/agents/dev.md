...
**Avant l'appel Figma** : lire `design-manifest/blocks/<slug>.json` (produit par `contrib`, voir `contrib.md` étape 8) pour retrouver, pour ce bloc, le `css_anchor` (classe CSS stable posée sur le bloc racine de la section) et le `block_type` réellement instancié. C'est le sélecteur à utiliser pour tout le SCSS de la section — jamais un sélecteur `nth-child`/structurel deviné dans le DOM, qui casse au moindre changement de composition. Si ce fichier ou l'entrée de la section attendue est absent, le signaler à l'Orchestrator (`missing_data`, `contrib` doit repasser) plutôt que de deviner un ancrage.
...
**Avant l'étape 5** : écrire/mettre à jour `design-manifest/blocks/<slug>-css.json` (clé = `figma_id`, repris de `blocks/<slug>.json`) avec les valeurs exactes retenues à l'étape 1 pour cette section — couleurs mappées aux tokens `_variables.scss`, typographie (px/famille/graisse/`line-height`/`letter-spacing`), spacing/radius, bornes de `clamp()` calculées. Ce fichier persiste ce que les étapes 1 et 3 viennent de déterminer, pour que les itérations 2-4 de ce bloc (et un futur rejeu en delta, §2 bis) n'aient pas à repartir d'un scan Figma complet.
...
0. **Traiter tout le bloc et tous ses sous-nœuds en une seule passe avant de compiler et de passer en revue.** Ne pas s'arrêter au premier écart visuel corrigé (ex. une photo) en laissant de côté les autres propriétés du même bloc (border-radius, badge, titre, tagline, description...) — corriger le bloc entier en live, puis compiler et vérifier une seule fois, plutôt que de boucler compile → constat → compile sur chaque sous-élément séparément. Retour d'expérience direct (The Place by CCI 45, 2e passe du 2026-08-26) : une première passe s'était arrêtée après la photo et le border-radius, laissant filer une erreur de hauteur d'image bien plus grave (15-30% d'écart, cf. point ci-dessous) qu'une passe complète aurait attrapée immédiatement.
...
- **Vérifier quel fichier CSS est réellement servi avant de faire confiance à une recompilation.** Certains projets ont un dossier de sortie CSS distinct du dossier édité/compilé à la main (ex. `css/theme.css` compilé par `npx sass`, mais `<link>` sur la page pointant vers `css-static/theme.css`) sans synchronisation automatique fiable — la recompilation "réussit" mais ne change rien au rendu. Inspecter `document.querySelector('link[href*="theme.css"]').href` (ou équivalent) pour confirmer le chemin réellement chargé avant de conclure qu'un correctif "ne prend pas" et de partir chercher un faux problème de spécificité CSS.
- **Un `clamp()`/une taille responsive partagée par un sélecteur commun (ex. `.wp-block-post-featured-image img`) peut être fausse pour une partie seulement des instances répétées**, si la maquette prévoit des ratios différents par carte (ex. carte 1 en paysage 608×414, carte 2 en portrait 412×571 sur The Place by CCI 45) — mesurer et corriger le `clamp()` **par instance** (`:nth-child`) avec la vraie valeur cible `get_metadata` de CHAQUE occurrence plutôt que de supposer qu'une seule formule couvre toutes les cartes d'une même boucle. Un écart de ce type (15 à 30%) se répercute en cascade sur tout le contenu sous l'image (titre/tagline/description/carte suivante) et peut ressembler à plusieurs petits bugs de marge alors que la cause est unique.
- **Auditer le conteneur/wrapper lui-même (position, largeur, padding/marge), pas seulement le contenu qu'il porte.** Une classe utilitaire générique (ex. `.container` Bootstrap avec sa gouttière `padding` par défaut) appliquée à un wrapper déjà positionné/dimensionné pixel-perfect par ailleurs peut introduire un décalage systémique (ex. +12px sur tout le contenu d'un bloc) sans équivalent dans la maquette. Comparer aussi le wrapper lui-même à `get_metadata` (x/y/width du nœud parent, marges internes type "Container:margin" en fin de nom Figma) — pas seulement les nœuds de texte/image qu'il contient.
...
- `design-manifest/blocks/<slug>.json` (frame↔bloc, `css_anchor`) est écrit par `contrib`, jamais par toi — tu le lis seulement. `design-manifest/blocks/<slug>-css.json` (valeurs CSS) est le tien : c'est toi qui l'écris, avant l'étape 5, jamais `contrib`.
