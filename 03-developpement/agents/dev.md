...
2. **Superposition pixel-perfect** pour valider : exporter le screenshot Figma du bloc à sa **largeur native exacte** (`maxDimension` = largeur du frame, ex. 1920px, jamais de scaling), le déposer temporairement comme fichier statique (ex. `wp-content/uploads/`, jamais dans le thème, jamais une entrée médiathèque — supprimé juste après usage), puis l'injecter en `<img>` `position:absolute` par-dessus le bloc live (aligné sur `section.getBoundingClientRect().top + scrollY`, largeur = `document.documentElement.clientWidth` — **pas** `window.innerWidth` ni la largeur brute du frame, pour compenser la barre de défilement du navigateur, sous peine d'un décalage systématique de ~15px), opacité ~50%. Screenshot du résultat.
...
- **Overlay pixel-perfect et barre de défilement** : toujours positionner l'image de superposition avec `document.documentElement.clientWidth`, jamais une largeur fixe (1920) ni `window.innerWidth` — la barre de défilement verticale du navigateur réduit la largeur réellement disponible pour le contenu d'une quinzaine de pixels, ce qui décale toute image positionnée sur la largeur brute du viewport.
