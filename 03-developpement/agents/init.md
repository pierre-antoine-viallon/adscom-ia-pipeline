...
Invoqué par l'Orchestrator en tout début de phase Développement. Prérequis : `agents/design-manifest/index.json` doit exister (produit par `spec`), `brief-projet.md` doit renseigner le chemin des infos base et du starterkit (si celui est absent, demande à l'utilisateur les informations nécessaires).
